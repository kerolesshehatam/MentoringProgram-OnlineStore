...
namespace OnlineStore.Catalog.Persistence.Repositories
{
    public class CategoryItemRepository : ICategoryItemRepository
    {
        ...
        public async Task<CategoryItem?> AddCatalogItemAsync(CategoryItem item)
        {
            if (item?.Id > 0)
            {
                return item;
        ...
        public async Task<IEnumerable<CategoryItem>?> GetCategoryItemsAsync(int categoryId)
        {
            return await _dbContext.CategoryItems.Where(t => t.CategoryId == categoryId).Include(t => t.Category).ToListAsync();
        }
        public async Task<CategoryItem?> UpdateCatalogItemAsync(CategoryItem item)
        {
            if (item?.Id <= 0)
